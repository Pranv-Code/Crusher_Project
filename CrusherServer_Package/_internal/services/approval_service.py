...
def build_change_details(cursor, req):
    ...
    try:
        if req_type == "sales_edit":
            ...
            cursor.execute("""
                SELECT s.sales_date, pt.party_name, COALESCE(p.product_name, 'Quarry Material') AS product_name, s.quantity_tons, s.unit, s.price
                FROM Sales s
                LEFT JOIN Product p ON s.product_id = p.product_id
                JOIN Party pt ON s.party_id = pt.party_id
                WHERE s.sales_id = %s
            ...
            cursor.execute("""
                SELECT p.production_date, COALESCE(pr.product_name, 'Quarry Material') AS product_name, p.quantity_tons, p.unit
                FROM Production p
                LEFT JOIN Product pr ON p.product_id = pr.product_id
                WHERE p.production_id = %s
            """, (ref_id,))
            ...
            cursor.execute("""
                SELECT s.sales_date, s.vehicle_number, s.quantity_tons, s.unit, COALESCE(p.product_name, 'Quarry Material') AS product_name, pt.party_name, s.loading_time
                FROM Sales s
                LEFT JOIN Product p ON s.product_id = p.product_id
                JOIN Party pt ON s.party_id = pt.party_id
                WHERE s.sales_id = %s
            ...
            sale = cursor.fetchone()
            if sale:
                loading_str = clean_time_str(sale.get("loading_time"))
...
def get_all_approvals():
    ...
    try:
        ...
        for req in requests_list:
            if "request_id" not in req and "approval_id" in req:
                req["request_id"] = req["approval_id"]
        ...
        if req["request_type"] == "vehicle":
            ...
            if status == "approved":
                ...
                if old_sale:
                    ...
                    unloading_val = clean_time_str(ref_data.get("unloading_time")) if (ref_data.get("unloading_time") is not None and str(ref_data.get("unloading_time")).strip() != "") else clean_time_str(old_sale.get("unloading_time"))
                    cursor.execute("""
                        UPDATE Sales SET
