...
def add_sales_bulk():
    ...
    common = body.get("common") if isinstance(body.get("common"), dict) else body
    rows   = body.get("rows", [])

    if not rows:
        return jsonify({"message": "No rows provided"}), 400

    default_party_id = common.get("party_id")
    default_sales_date = common.get("sales_date")
    default_site = capitalize_words(common.get("site", ""))

    # Validate that every row has a party_id and sales_date (either from common or per row)
    for idx, r in enumerate(rows, 1):
        r_party = r.get("party_id") or default_party_id
        r_date = r.get("sales_date") or default_sales_date
        if not r_party or not r_date:
            return jsonify({"message": f"Row #{idx}: Party and Sales Date are required (either in Common Information or per row)."}), 400
    ...
    try:
        # Verify default party if present
        if default_party_id:
            cursor.execute("SELECT party_name FROM Party WHERE party_id=%s", (default_party_id,))
            if not cursor.fetchone():
                return jsonify({"message": "Selected Common Party not found"}), 404
        ...
        for idx, row in enumerate(rows):
            ...
            if inv_mode == "COMMON_POOL":
                ...
                if (avail + 0.0001) < float(qty):
                    errors.append(f"{row_label}: Insufficient Stock for product (Available: {avail:.2f} MT, Requested: {float(qty):.2f} MT)")
                    continue
...
def delete_sale(id):
    ...
    try:
        ...
        if user_role == "Clerk":
            ...
            cursor.execute(sql_del_appr, (user_id, str(id)))
            log_activity(
                user_id,
...
def update_sale(id):
    ...
    try:
        ...
        if user_role == "Clerk":
            ...
            cursor.execute(sql_edit_appr, (user_id, str(id), json.dumps(data)))
            log_activity(
                user_id,
                request.user.get("username", "Clerk"),
                user_role,
        ...
        unloading_time_val = data.get("unloading_time") if (data.get("unloading_time") is not None and str(data.get("unloading_time")).strip() != "") else _format_time(old_sale.get("unloading_time"))
